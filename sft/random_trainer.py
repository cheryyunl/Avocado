...
class RWSFTTrainer(SFTTrainer):
    def __init__(
        ...
        **trainer_args
        ...
        self.n_gpu_per_task = self.accelerator.num_processes // self.n_tasks
        self.w = torch.clamp(torch.randn(self.n_tasks, device='cuda'), 0.3, 0.7) 
    ...
    def training_step(
        ...
        random_w = F.softmax(self.w, dim=-1)
        ...
        task_id = inputs.pop("task_id")[0].item()
        ...
        with self.compute_loss_context_manager():
            ...
            loss = orig_loss * random_w[task_id] * self.n_tasks
        ...
        all_losses = all_losses / self.n_gpu_per_task

        if self.step_count % self.update_frequency == 0:
            if self.accelerator.is_main_process:
                self.w.data = torch.clamp(torch.randn(self.n_tasks, device=self.w.device), 0.3, 0.7) 
            
            self.accelerator.wait_for_everyone()
            torch.distributed.broadcast(self.w, src=0)
        
            wandb.log({
                **{f'task_{task_id}_weight': random_w[task_id].item() for task_id in range(self.n_tasks)},
