...
class FAMOSFTTrainer(SFTTrainer):
    def __init__(
        ...
        **trainer_args
        ...
        self.rejected_loss_type=rejected_loss_type
        self.beta = beta
    ...
    def update_famo_weights(self, prev_loss, curr_loss):
        ...
        '''
        max_vals = torch.maximum(prev_loss.max(dim=0, keepdim=True)[0], 
                                curr_loss.max(dim=0, keepdim=True)[0]) + 1e-8
        
        prev_norm = prev_loss / max_vals
        curr_norm = curr_loss / max_vals
    
        delta = (prev_norm - self.min_losses + 1e-8).log() - (curr_norm - self.min_losses + 1e-8).log()
    ...
    def eval_loss(self, model, inputs, return_outputs=True, num_items_in_batch=None):
        """
    ...
    def training_step(
        ...
        with self.compute_loss_context_manager():
            orig_loss, outputs = self.compute_loss(model, inputs, return_outputs=True)
            if self.rejected_ids is not None and task_id in self.rejected_ids:
                if self.rejected_loss_type == 'npo':
                    labels = inputs.input_ids
                    rejected_loss_current = get_batch_loss(outputs.logits, labels) 
                    with torch.no_grad():
                        rejected_outputs_oracle = self.oracle_model(**inputs)
                    rejected_logits_oracle = rejected_outputs_oracle.logits
                    rejected_loss_oracle = get_batch_loss(rejected_logits_oracle, labels)
                    rejected_loss_current = torch.clamp(rejected_loss_current, min=1e-8)
                    rejected_loss_oracle = torch.clamp(rejected_loss_oracle, min=1e-8)
            
                    neg_log_ratios = rejected_loss_current - rejected_loss_oracle            
                    beta = torch.tensor(self.beta, device=orig_loss.device)
                    final_loss = -F.logsigmoid(beta * neg_log_ratios).mean() * 2 / beta
                    final_loss = torch.clamp(self.loss_scale * final_loss, min=1e-8, max=1e-3)
                else:
                    final_loss = - self.loss_scale * torch.log(1 + orig_loss)
            else:
                final_loss = orig_loss
            loss = final_loss * famo_w[task_id]
        ...
        all_losses[task_id] = final_loss.detach()
        ...
        if self.step_count > 0 and self.step_count % self.famo_update_frequency == 0:
            ...
            with torch.no_grad():
                new_loss, new_outputs = self.eval_loss(model, self.prev_inputs)
            if self.rejected_ids is not None and prev_task_id in self.rejected_ids:
                if self.rejected_loss_type == 'npo':
                    new_labels = self.prev_inputs['input_ids']
                    new_loss_current = get_batch_loss(new_outputs.logits, new_labels) 
                    with torch.no_grad():
                        new_outputs_oracle = self.oracle_model(**self.prev_inputs)
                    new_logits_oracle = new_outputs_oracle.logits
                    new_loss_oracle = get_batch_loss(new_logits_oracle, new_labels)
                    
                    new_loss_current = torch.clamp(new_loss_current, min=1e-8)
                    new_loss_oracle = torch.clamp(new_loss_oracle, min=1e-8)
                    
                    new_neg_log_ratios = new_loss_current - new_loss_oracle                    
                    beta = torch.tensor(self.beta, device=new_loss.device)
                    new_loss = -F.logsigmoid(beta * new_neg_log_ratios).mean() * 2 / beta

                    new_loss = torch.clamp(self.loss_scale * new_loss, min=1e-8, max=1e-3)
                else:
